...
export async function getCabinPrice(
  ...
  return (data as Pick<Cabin, "regularPrice" | "discount">) ?? null;
}
...
type BookingRow = Omit<Booking, "cabin"> & {
  cabins: Cabin;
...
export async function getBookings(guestId: number): Promise<Booking[]> {
  ...
  return bookings.map((booking) => {
    const { cabins, ...rest } = booking;
    return {
      ...rest,
      observations: booking.observations ?? undefined,
      hasBreakfast: booking.hasBreakfast ?? undefined,
      extrasPrice: booking.extrasPrice ?? undefined,
      cabinPrice: booking.cabinPrice ?? undefined,
      isPaid: booking.isPaid ?? undefined,
      cabin: cabins,
    };
  });
}
